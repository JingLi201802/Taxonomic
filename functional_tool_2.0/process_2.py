    ...
    if request.method == "POST":
        if request.files:
            ...
            if xml_file.filename.split('.')[-1] == 'xml':
                ...
                TaxonomyExtractPDF_2.get_configurations()
                TaxonomyExtractPDF_2.pdf_to_text(app.config['DOWNLOAD_FOLDER'] + '/' + pdf_file.filename)
                txtCrawl.get_csv_output_test((app.config['DOWNLOAD_FOLDER'] + '/' + pdf_file.filename[:-4] +'.txt'), TaxonomyExtractPDF_2.direct_mappings, TaxonomyExtractPDF_2.get_output_path(pdf_file.filename[:-4]))
